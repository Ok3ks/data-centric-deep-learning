...
from src.system import MNISTDataModule, DigitClassifierSystem, MNISTDataModuleAug
...
class DigitClassifierFlow(FlowSpec):
  ...
  def offline_test(self):
    ...
    log_file = join(Path(__file__).resolve().parent.parent, 
      f'logs/aug/mlp', 'offline-test-results.json')
  ...
  def directionality_test(self):
    ...
    log_file = join(Path(__file__).resolve().parent.parent, 
      f'logs/aug/mlp', 'directionality-test-results.json')
