...
class MNISTDataModuleAug(pl.LightningDataModule):
  ...
  def __init__(self, config):
    ...
    if config.system.name == 'resnet':
      image_transform = transforms.Compose([transforms.Resize(224),
                      transforms.RandomRotation(45),
                      transforms.AutoAugment(),
                      #transforms.Lambda(lambd= torch.from_numpy(random_noise()).float()),
                      #: torch.from_numpy(random_noise(x)).float()
                      transforms.ToTensor(),])
      in_transforms = transforms.Compose([transforms.Resize(224), transforms.ToTensor()])
    else : 
      image_transform = transforms.Compose([transforms.RandomRotation(45),
                      transforms.AutoAugment(),
                      #transforms.Lambda(lambd= torch.from_numpy(random_noise()).float()),
                      #: torch.from_numpy(random_noise(x)).float()
                      transforms.ToTensor(),])
    ...
    # In MNISTDataModule...
    train_dataset_aug = datasets.MNIST(config.system.data.root, download = True, train = True, transform = image_transform)
    train_dataset = datasets.MNIST(config.system.data.root, download = True, train = True, transform = in_transforms)
    test_dataset = datasets.MNIST(config.system.data.root, download = True,train = False, transform = in_transforms)
    ...
    _train_dataset = Subset(train_dataset_aug, indices[:-dev_size])
...
class DigitClassifierSystem(pl.LightningModule):
  ...
  def get_model(self):
    ...
    elif self.config.system.model.name == 'resnet':
      model = nn.Sequential(
  ...
  def forward(self, image):
    if self.config.system.model.name == 'resnet':
      image = image.expand(32,3,224,224) # flatten the image
    else:
