...
class DigitClassifierFlow(FlowSpec):
  ...
  def start(self):
    ...
    self.lr = [1e-5, 1e-4, 1e-3]

    # for each learning_rate, we will train a model
    self.next(self.init_and_train, foreach='lr')
  ...
  def init_and_train(self):
    ...
    config.system.optimizer.lr = self.input
  ...
  def find_best(self, inputs):
    ...
    self.best_lr = inputs[best_index].lr[best_index]
  ...
  def offline_test(self):
    ...
    results['best_lr'] = self.best_lr
    results['width'] = self.widths

    pprint(results)

    log_file = join(Path(__file__).resolve().parent.parent, 
      f'logs/hparam_flow_2', 'offline-test-results.json')
