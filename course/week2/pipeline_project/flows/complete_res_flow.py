...
class DigitClassifierFlow(FlowSpec):
  ...
  config_path = Parameter('config', 
    help = 'path to config file', default='./configs/resnet.json')
  ...
  def offline_test(self):
    ...
    log_file = join(Path(__file__).resolve().parent.parent, 
      f'logs/resnet50', 'offline-test-results.json')
  ...
  def integration_test(self):
    """Performs integration test on handwritten digits"""
    ...
    log_file = join(Path(__file__).resolve().parent.parent,
      f'logs/resnet50', 'integration-test.json')

    self.next(self.regression_test)

  @step
  def regression_test(self):
    """Performs regression test on prior examples"""
    ...
    log_file = join(Path(__file__).resolve().parent.parent,
      f'logs/resnet50', 'regression_test.json')
  ...
  def directionality_test(self):
    ...
    log_file = join(Path(__file__).resolve().parent.parent, 
      f'logs/aug/linear', 'directionality-test-results.json')
